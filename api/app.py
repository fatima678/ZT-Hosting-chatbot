...
# --- STEP 1: LIGHTWEIGHT EMBEDDINGS SETUP ---
# Ye API use karta hai, isliye Vercel ki RAM crash nahi hogi
embeddings = HuggingFaceHubEmbeddings(
    huggingfacehub_api_token=os.environ.get("HUGGINGFACE_TOKEN"),
    model="sentence-transformers/all-MiniLM-L6-v2"
)

# Vector DB connect karein (Path ko safe banaya gaya hai)
...
def get_llm():
    return ChatGroq(
        groq_api_key=os.environ.get("GROQ_API_KEY"), 
        model_name="llama-3.1-8b-instant", 
        temperature=0 # 0 means serious and accurate responses
    )
...
async def ask_bot(request: Request):
    try:
        ...
        # 1. GREETINGS HANDLER
        if re.search(r'^(h+[iy]+|h+e+l+o+|a+o+a+|s+a+l+a+m+)', user_query_lower):
            return {"answer": "Hello! Welcome to ZT Hosting. How can I assist you with our hosting services today?"}

        # 2. VECTOR SEARCH (Relevant data uthana)
        docs = vector_db.similarity_search(user_input, k=3)
        context_text = "\n\n".join([doc.page_content for doc in docs])

        # 3. MATURE SYSTEM PROMPT
        llm = get_llm()
        system_prompt = (
            "You are a Senior ZT Hosting Support Executive. "
            "Your tone must be professional, direct, and helpful. "
            "STRICT RULES: "
            "1. Do not use filler words like 'Okay' or 'I understand'. "
            "2. ALWAYS provide features in exactly 3 bullet points. "
            "3. ALWAYS use a Markdown Table if comparing two or more plans. "
            "4. Use **bold** for prices, plan names, and key technical terms. "
            "5. If information is not in context, say: 'I apologize, but I don't have that specific information. Please contact our 24/7 support team.' "
        ...
        chain = prompt | llm
        # Invoke with empty dict because variables are already in context_text
        response = chain.invoke({})
        
        return {"answer": response.content}

    except Exception as e:
        return {"answer": f"I'm sorry, I'm having trouble processing that. (Error: {str(e)})"}

# --- Routes for UI (Baqi saare routes as it is hain) ---
@app.get("/", response_class=HTMLResponse)
async def get_home():
    if HTML_PATH.exists(): return HTML_PATH.read_text(encoding="utf-8")
    return "<h1>Chatbot Home</h1>"
...
# (Yahan aapke add-faq, update-faq, settings wagera ke baqi saare functions hain...)
# [Note: Maine space bachane ke liye baqi functions repeat nahi kiye, 
# lekin wo aapke code mein niche waise hi rahenge jaise pehle the]
