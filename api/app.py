...
# --- Path Configuration (Fixed & Cleaned) ---
CURRENT_FILE = Path(__file__).resolve()
API_DIR = CURRENT_FILE.parent
BASE_DIR = API_DIR.parent 
...
# --- Main Chat Logic (Updated for Sir's Requirements) ---
@app.post("/ask")
async def ask_bot(request: Request):
    try:
        data = await request.json()
        ...
        if re.search(r'^(h+[iy]+|h+e+l+o+|a+o+a+|s+a+l+a+m+|kia hal|how are you)', user_query_lower):
            return {"answer": "Hello! Welcome to ZT Hosting. I am your specialized assistant. How can I assist you with our web hosting or domain services today?"}

        # 2. IRRELEVANT FILTER (Requirement: Strictly Hosting Only)
        hosting_keywords = ['host', 'domain', 'server', 'plan', 'price', 'email', 'wp', 'wordpress', 'cpanel', 'ssl', 'vps', 'shared', 'package', 'cost', 'pkr', 'buy', 'purchase', 'zt', 'order']
        
        if not any(word in user_query_lower for word in hosting_keywords):
            return {"answer": "I apologize, but I am designed to assist only with ZT Hosting related queries. Please ask about our hosting plans, domains, or servers."}

        # 3. CONTEXT SEARCH (Local File Retrieval)
        context_text = ""
        user_words = [word for word in user_query_lower.split() if len(word) > 2]

        if DATA_DIR.exists():
            all_files = [f for f in os.listdir(DATA_DIR) if f.endswith(".txt")]
            # Filename matching for priority
            matched_files = [f for f in all_files if any(w in f.lower() for w in user_words)]
        ...
        # 4. SYSTEM PROMPT (Strict Formatting & Context Rules)
        llm = get_llm()
        system_prompt = (
            "You are a Senior ZT Hosting Executive. "
            "1. ONLY answer hosting questions using the provided context. "
            "2. If the user asks something irrelevant, strictly refuse. "
            "3. Format: Short intro, EXACTLY 3 bullet points for info, then a small depth paragraph. "
            "4. Use **bold** for prices and plan names. "
            "5. If information is missing from context, ask them to contact support@zthosting.com."
        )
...
async def add_faq(request: Request):
    ...
    if not question or not answer: return {"status": "error", "message": "Required fields missing"}
    
    try:
        supabase.table("manual_faqs").insert({"question": question.lower(), "answer": answer}).execute()
        return {"status": "success"}
    except Exception: return {"status": "error", "message": "Database error"}
...
async def update_faq(request: Request):
    ...
    data = await request.json()
    faq_id = data.get("id")
    if not faq_id: return {"status": "error", "message": "Invalid FAQ ID"}
